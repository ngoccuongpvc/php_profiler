...
static int determine_clock_source(int clock_use_rdtsc) {
    ...
    return PHP_PROFILER_CLOCK_MACH;
#elif defined(__powerpc__) || defined(__ppc__)
    return PHP_PROFILER_CLOCK_TSC;
#elif defined(__s390__) // Covers both s390 and s390x.
    return PHP_PROFILER_CLOCK_TSC;
#elif defined(__ARM_ARCH)
    return PHP_PROFILER_CLOCK_GTOD;
#elif defined(PHP_WIN32)
    return PHP_PROFILER_CLOCK_QPC;
#else
    struct timespec res;
...
static zend_always_inline uint64 time_milliseconds(int source, double timebase_factor)
{
    ...
    if (source == PHP_PROFILER_CLOCK_TSC) {
        return time_milliseconds_tsc_query() / timebase_factor;
    } else if (source == PHP_PROFILER_CLOCK_GTOD) {
        return time_milliseconds_gtod();
    } else if (source == PHP_PROFILER_CLOCK_CGT) {
